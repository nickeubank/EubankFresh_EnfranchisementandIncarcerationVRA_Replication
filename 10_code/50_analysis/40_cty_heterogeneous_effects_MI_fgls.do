        ...
        foreach dv_num in 1 2  {
            ...
            forvalues sample = 1/`subsample_count' {
                ...
                * First, FGLS weights
                forvalues i = 1/5 {
                    ...
                    display "`i'"
                    reg  ${dv`dv_num'} ${nonpar} ${controls} year_* countyfe_* if `subsample' ///
                            , cluster($cluster)
                    predict double e if `subsample', resid
                    * gen double e = ${dv`dv_num'} - yhat
                    sum e
                    assert r(mean) <= r(sd)/5
                    gen ln_e2 = log(e^2)
                    reg ln_e2 countyfe_* if `subsample'
                    predict logged_fgls_weight if `subsample', xb
                    generate fgls_weight = 1/(exp(logged_fgls_weight)^0.5) if `subsample'
                    assert fgls_weight >= 0
                    sum  fgls_weight, d
                    assert r(max) < 1
            ...
            * Define labels
        	...
        	esttab ///
        		...
        		est_3 ///
        		est_4 ///
        		using "$NewJimCrow/50_results/heterogeneous_`measure_label'_`rate'_rate_fgls_MI_`drops'.tex", ///
